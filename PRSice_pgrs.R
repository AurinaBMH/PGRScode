...
# clumping
# BPJ: Note if using original dosage data that PRSice is unable to apply clumping or pruning at the moment.
# Clumping OK for PLINK BED/BIM/FAM files. We have already performed LD based SNP pruning. 
clump.snps <- T
clump.p1 <- 1
clump.p2 <- 1
clump.r2 <- 0.1
   ...
   if(binary.target){
    ...
    cat(" ################################# \n # \n #   Quantiles Plots \n # \n ################################# \n")
      ...
      if(!binary.target){
        ...
        # names(scores.internal) <- c("ID", "SCORE")
        for.quantiles <- merge(x = phen.file.internal, by.x = "ID", y = scores.internal, by.y = "IID")
        for.quantiles$quantiles <- as.numeric(cut(for.quantiles$pT_0.05, breaks = quantile(for.quantiles$pT_0.05, probs = seq(0, 1, 1/num.quantiles)), include.lowest=T))
        for.quantiles$quantiles <- factor(for.quantiles$quantiles, levels = c(quant.ref, seq(1, num.quantiles, 1)[seq(1, num.quantiles, 1) != quant.ref]))
